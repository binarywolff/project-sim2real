...
def create_model(env, seed, lr=0.0003):
    return TD3("MlpPolicy", env, verbose=1, learning_rate=lr, seed=seed, device='auto')

def load_model(file, env):
    return TD3.load(file, env=env)
...
def train(env, seed, lr=0.0003, total_timesteps=200_000, log_dir='./tmp/gym', env_id='Humanoid-v3'):
    ...
    model.save(f"models/TD3_{env_id}")
...
def plot_results(log_folder, title="Learning Curve"):
    ...
    plt.savefig(f"reward_plot/td3_reward.png", dpi=300, bbox_inches="tight")