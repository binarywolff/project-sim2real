...
def create_model(env, seed, lr=0.0003):
    return PPO("MlpPolicy", env, verbose=1, learning_rate=lr, seed=seed, device='auto')

def load_model(file, env):
    return PPO.load(file, env=env)

def train(env, seed, lr=0.0003, total_timesteps=200_000, log_dir='./tmp/gym', env_id='CustomHopper-source-v0'):
...
def test(env, model_file, render=False, test_episodes=50, log_dir='./tmp/gym'):
...
def plot_results(log_folder, title="Learning Curve"):
    ...
    plt.savefig(f"reward_plot/ppo_reward.png", dpi=300, bbox_inches="tight")